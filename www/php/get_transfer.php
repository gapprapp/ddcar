<?php
    ...
    if(isset($_POST['date'])){
        ...
        ON s.shop_id = w.shop_id WHERE s.date_time BETWEEN '$date' AND '$date_to' AND s.tran_number NOT LIKE '%$txt%' LIMIT $start,30";
        ...
        AND s.tran_number NOT LIKE '%$txt%' ORDER BY s.tran_id DESC LIMIT $start,30";
    }else{
        $query = "SELECT s.tran_id,s.tran_number,w.shop_name,s.date_time FROM transfer_in s INNER JOIN shop_detail w
        ON s.shop_id = w.shop_id WHERE s.tran_number NOT LIKE '%$txt%' ORDER BY s.tran_id DESC LIMIT $start,30";
    }    
